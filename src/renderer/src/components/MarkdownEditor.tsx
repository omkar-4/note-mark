    ...
    <>
      ...
      <header className="hidden sm:flex flex-row items-center justify-start gap-4 flex-wrap mb-2">
        {/* <button
          ...
          // onClick={() => editorRef.current?.setMarkdown(md_template)}
        >
          Start with Template
        </button>
        ...
        <button
          className="border hover:border-transparent hover:bg-[#F57F00] active:bg-[#9c5101] rounded-full px-3 py-0.5 text-sm"
          // onClick={() => {
          //   let data = editorRef.current?.getMarkdown()
          //   console.log(data)
          // }}
        >
          Extract to Console
        ...
        <button
          className="border hover:border-transparent hover:bg-[#F57F00] active:bg-[#9c5101] rounded-full px-3 py-0.5 text-sm"
          // onClick={() => editorRef.current?.setMarkdown('')}
        >
          Clear
        </button> */}
      ...
      <MDXEditor
        ...
        contentEditableClassName="outline-none min-h-svh max-w-none text-lg px-8 py-5 caret-zinc-100 prose prose-invert prose-p:my-0 prose-headings:tracking-wider prose-p:leading-normal prose-headings:my-4 prose-bockquote:my-4 prose-ul:my-2 prose-li:my-0 prose-code:px-1 prose-code:text-green-500 prose-code:before:content-[''] prose-code:after:content-['']"
        plugins={[
          headingsPlugin(),
          listsPlugin(),
          quotePlugin(),
